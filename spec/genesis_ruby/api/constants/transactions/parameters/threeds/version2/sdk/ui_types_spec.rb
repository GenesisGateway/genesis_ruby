require 'genesis_ruby/api/constants/transactions/parameters/threeds/version2/sdk/ui_types'
require 'spec/genesis_ruby/shared_examples/api/constants/common_examples'

RSpec.describe GenesisRuby::Api::Constants::Transactions::Parameters::Threeds::Version2::Sdk::UiTypes do

  include_examples 'common constants examples', 'single_select'

end

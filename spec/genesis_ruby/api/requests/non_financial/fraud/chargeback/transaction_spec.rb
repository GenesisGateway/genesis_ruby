RSpec.describe GenesisRuby::Api::Requests::NonFinancial::Fraud::Chargeback::Transaction do
  let(:config) do
  ...
  describe 'when initialized' do
    it 'with valid endpoint' do
      expect(request.api_config[:url]).to eq 'https://staging.gate.emerchantpay.net:443/chargebacks'
    end

    it 'with XML root element' do
      expect(request.build_document).to include '<chargeback_request>'
    end
